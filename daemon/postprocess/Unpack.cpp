...
void UnpackController::CreateUnpackDir()
{
	...
	if (g_Options->GetUseTempUnpackFolder())
	{
...
bool UnpackController::Cleanup()
{
	...
	if (m_unpackOk && g_Options->GetUseTempUnpackFolder())
	{
		// moving files back
	...
	CString errmsg;
	if (ok && g_Options->GetUseTempUnpackFolder() && !FileSystem::DeleteDirectoryWithContent(m_unpackDir, errmsg))
	{
